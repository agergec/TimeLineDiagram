...
function getAutoBoxColor(laneId) {
    const laneIndex = app.diagram.lanes.findIndex(l => l.id === parseInt(laneId));
    if (laneIndex === -1) return '#6366F1';

    // Base color from palette
    const baseColor = PALETTE[laneIndex % PALETTE.length];

    // Shift slightly based on box count to distinct
    const laneBoxes = app.diagram.getBoxesForLane(laneId);
    // Use modulo to cycle through shifts
    const shift = ((laneBoxes.length % 5) * 15) - 30;
